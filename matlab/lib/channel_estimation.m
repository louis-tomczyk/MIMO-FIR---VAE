...
function [thetas_est, H_est_f] = extract_thetas_est(H_est,k,H_est_f,caps)
...
if strcmpi(caps.method.thetas,'fft')
    H_f0            = squeeze(H_est_f(k,:,:,caps.tap));
...
elseif strcmpi(caps.method.thetas,'eig')
    Tr          = sum(eig(Hest));
    D           = prod(eig(Hest));
    C           = Tr/2/sqrt(D);
