...
for m = 1:Nrea
    for k = 1:NSNRs
        for j = 1:Nths
            N0      = 1./SNRs(k);
            dtheta  = sqrt(12*N0)*rand;
            dthetas(m,k,j) = dtheta;

            phi = sqrt(Nsymbs*var_phi)*randn;

            R       = [ [cos(thetas(j)+dtheta),sin(thetas(j)+dtheta)];...
                        [-sin(thetas(j)+dtheta),cos(thetas(j)+dtheta)]]*...
                        exp(1i*phi);
...
mean_CthHats= mean(cos(ThHats));
std_CthHats = std(cos(ThHats));


mean_dCth   = mean(cos(ThHats)-cos(ThExt));
std_dCth    = std(cos(ThHats)-cos(ThExt));
...
%         set(gca, "YScale", 'log')
    ...
    f3 = figure(3);
        hold on
        h3 = errorbar(SNRsdB, mean_dCth(:,:,k),std_dCth(:,:,k),...
            'Color', colors(color_idx, :), ...
            'LineStyle', line_styles{line_style_idx}, ...
            'Marker', markers{marker_idx}, ...
            'MarkerFaceColor', colors(color_idx, :),...
